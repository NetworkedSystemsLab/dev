...
{
 if(strcmp(file, STEERING) == 0)
 {
  if(65 > val || 130 < val) return;
 }
 if(strcmp(file, THROTTLE) == 0)
 {
  if(1000 > val || 2000 < val) return;
 }
 //open the file
...
/*               Throttle = %lf , Steering = %lf\*/
 ...
 if(car.throttle > 2000 || car.throttle < 1000)
 {
    std::cout << "\n               ";
    std::cout << "THROTTLE IS OUT OF RANGE; NOT WRITING TO MOTOR\n               ";
 }
 if(car.steering > 130 || car.steering < 65)
 {
  std::cout << "\n               ";
  std::cout << "STEERING IS OUT OF RANGE; NOT WRITING TO STEERING SERVO\n";
 }
}
